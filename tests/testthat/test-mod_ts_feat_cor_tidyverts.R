...
test_that("ts_feat_cor_tidyverts_server - reactives and output updates", {
  testServer(ts_feat_cor_tidyverts_server,
    args = list(
      tsbl_vars = reactive(tsbl_vars),
      tsbl_vars_average = reactive(tsbl_vars_average)
    ),
    {
      # ts_feat_cor_tidyverts_server with typical user inputs ====

      # Use to avoid skip message due to empty test, replace it with real tests
      expect_true(TRUE)

      # -- Sample Code for reference --
      # Set input for ts_feat_cor_tidyverts
      # select_factors <- c("CR", "QR")
      # session$setInputs(
      #   factor_groups = "Financial Risk",
      #   select_factors = select_factors,
      #   ts_feat_cor_tidyverts = 1 # Non-null value indicating use click button
      # )
      # Check ts_feat_cor_tidyverts()
      # ts_feat_cor_tidyverts <- ts_feat_cor_tidyverts()
      # expect_s3_class(ts_feat_cor_tidyverts, "data.frame")
      # expect_fields <- c(c("date", "period", "stkcd", "indcd"), select_factors)
      # actual_fields <- names(ts_feat_cor_tidyverts
      # expect_true(all(actual_fields %in% expect_fields))
      # expect_true(nrow(ts_feat_cor_tidyverts >= 0)
      # Check output
...
test_that("ts_feat_cor_tidyverts_app - Module App works", {
  skip_on_cran()
  skip_on_ci()
  skip_on_covr()
  ...
  withr::with_file(test_app_file, {
    ...
    # ts_feat_cor_tidyverts_app with typical user inputs ====
    ...
    # select_factors <- c("CR", "QR")
    # app$setInputs(
    #   `ts_feat_cor_tidyverts_module-factor_groups` = "Financial Risk",
    #   `ts_feat_cor_tidyverts_module-factors_in_group` = select_factors,
    #   `ts_feat_cor_tidyverts_module-ts_feat_cor_tidyverts` = "click",
    #   timeout_ = 1000 * 10
    # )
