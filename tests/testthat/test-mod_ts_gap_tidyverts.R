# Tests for module of ts_gap_tidyverts  ----

# context("Tests for module of ts_gap_tidyverts")
...
test_that("ts_gap_tidyverts_server - reactives and output updates", {
  testServer(ts_gap_tidyverts_server,
                ...
                tsbl_vars_average = reactive(tsbl_vars_average)),
    {
      ...
      # Set input for ts_gap_tidyverts
      # select_factors <- c("CR", "QR")
      # session$setInputs(
      #   factor_groups = "Financial Risk",
      #   select_factors = select_factors,
      #   ts_gap_tidyverts = 1 # Non-null value indicating use click button
      # )
      # Check ts_gap_tidyverts()
      # ts_gap_tidyverts <- ts_gap_tidyverts()
      # expect_s3_class(ts_gap_tidyverts, "data.frame")
      # expect_fields <- c(c("date", "period", "stkcd", "indcd"), select_factors)
      # actual_fields <- names(ts_gap_tidyverts
      # expect_true(all(actual_fields %in% expect_fields))
      # expect_true(nrow(ts_gap_tidyverts >= 0)
      # Check output
...
test_that("ts_gap_tidyverts_app - Module App works", {
  skip_on_cran()
  skip_on_ci()
  skip_on_covr()

  withr::local_tempdir("test_ts_gap_tidyverts_app")
  test_app_file <- "app.R"
  withr::with_file(test_app_file, {
    ...
    # ts_gap_tidyverts_app with typical user inputs ====
    ...
    # select_factors <- c("CR", "QR")
    # app$setInputs(
    #   `ts_gap_tidyverts_module-factor_groups` = "Financial Risk",
    #   `ts_gap_tidyverts_module-factors_in_group` = select_factors,
    #   `ts_gap_tidyverts_module-ts_gap_tidyverts` = "click",
    #   timeout_ = 1000 * 10
    # )
