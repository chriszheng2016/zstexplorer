...
test_that("cs_cor_GGally_server - reactives and output updates", {
  testServer(cs_cor_GGally_server,
    args = list(csbl_vars = reactive(csbl_vars)),
    {
      ...
      #   cs_cor_GGally = 1 # Non-null value indicating use click button
      # )
      # Check cs_cor_GGally()
      # cs_cor_GGally <- cs_cor_GGally()
      # expect_s3_class(cs_cor_GGally, "data.frame")
      # expect_fields <- c(c("date", "period", "stkcd", "indcd"), select_factors)
      # actual_fields <- names(cs_cor_GGally
      # expect_true(all(actual_fields %in% expect_fields))
      # expect_true(nrow(cs_cor_GGally >= 0)
      # Check output
...
test_that("cs_cor_GGally_app - Module App works", {
  skip_on_cran()
  skip_on_ci()
  skip_on_covr()

  withr::local_tempdir("test_cs_cor_GGally_app")
  test_app_file <- "app.R"
  withr::with_file(test_app_file, {
    ...
    # cs_cor_GGally_app with typical user inputs ====

    # -- Sample Code for reference --
    # select_factors <- c("CR", "QR")
    # app$setInputs(
    #   `cs_cor_GGally_module-factor_groups` = "Financial Risk",
    #   `cs_cor_GGally_module-factors_in_group` = select_factors,
    #   `cs_cor_GGally_module-cs_cor_GGally` = "click",
    #   timeout_ = 1000 * 10
    # )
