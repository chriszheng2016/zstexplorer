...
estimate_ncp_PCA <- missMDA::estim_ncpPCA(df_pca_raw[, -1], method.cv = "gcv")
...
impute_res <- missMDA::imputePCA(df_pca_raw[, -1], ncp = estimate_ncp_PCA$ncp)

df_pca <- impute_res$completeObs %>%
  tibble::as_tibble(rownames = "id") %>%
  dplyr::bind_cols(indcd = df_pca_raw$indcd) %>%
  dplyr::select(indcd, everything()) %>%
...
```{r, echo = FALSE}
hcpc_res <- HCPC(pca_res, nb.clust = -1, graph = FALSE)
...
### Individuals in clusters {.unnumbered}
```{r}
