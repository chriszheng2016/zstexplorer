...
focus_indnames <- code2name(focus_indcds)
...
stretch_long_tsbl_focus_stock <- long_tsbl_focus_stock_train %>%
...
strench_long_tsbl_focus_stock <- long_tsbl_focus_stock %>%
  tsibble::stretch_tsibble(.init = 4, .step = 1) %>%
  dplyr::filter(.data$.id != max(.data$.id))
...
# long_tsbl_focus_stock_ets_cv_forecast %>%
#     dplyr::filter(.id == 4) %>%
#     autoplot(strench_long_tsbl_focus_stock, level = NULL, show_gap = FALSE) +
#     autolayer(strench_long_tsbl_focus_stock, value, color = "gray", linetype = "dotted") +
#     facet_grid(stkcd ~ variable, scale = "free")
# 
# long_tsbl_focus_stock_ets_cv_forecast %>%
#     #dplyr::filter(stkcd == "000157", variable == "QR") %>%
...
long_tsbl_focus_stock_ets_cv_forecast %>%
    autoplot(strench_long_tsbl_focus_stock, level = NULL,
             show_gap = TRUE) +
    autolayer(strench_long_tsbl_focus_stock, value, color = "black") +
    facet_grid(stkcd ~ variable, scale = "free")
...
nest_tsbl_focus_stock_train_diff <-
  ...
  dplyr::mutate(
    tsbl_variable =
      list(
        difference_tsbl(.data$tsbl_variable, value_field = "value",
...
nest_tsbl_focus_stock_train_diff <-
  ...
  dplyr::mutate(
    tsbl_variable =
      list(
        difference_tsbl(.data$tsbl_variable, value_field = "sdiff",
...
long_tsbl_focus_stock_arima_forecast <- long_tsbl_focus_stock_arima_fit %>%
  forecast(h = test_periods)

long_tsbl_focus_stock_arima_forecast %>%
  autoplot(long_tsbl_focus_stock_train) +
  autolayer(long_tsbl_focus_stock_test, value, color = "black") +
  facet_grid(stkcd ~ variable, scale = "free")
...
long_tsbl_focus_stock_model_cv_point_accuracy %>%
  formattable::formattable(
    list(
      ~highlight_formatter,
      RMSE = formattable::color_tile("white", "lightblue")
    )
  ) %>%
  formattable::as.datatable(
